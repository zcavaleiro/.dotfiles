...
check_var() {
    local var_name=$1
    if [ -z "${!var_name-}" ]; then
        echo "Erro: A variável $var_name não está definida."
        exit 1
    fi
}
...
echo '# -------------- zcavaleiro:dotfiles Instaling defined OS packages and dependencies ---------------'
...
# Instaling apps with source signature
...
# Application or file download, extract and creation of a desktop shortcut from extracted dir
...
for application in $APPLICATIONS; do
    ...
    # Create shortcut in "desktop" -  "Área de trabalho"
...
# Creating the symblinks to the system
cd ~/.dotfiles
ln -sf ~/.dotfiles/.bashrc ~/.bashrc
ln -sf ~/.dotfiles/.gitconfig ~/.gitconfig


# Making pull from recent changes in .dofiles directory, use of alias for shortcut
...
# Apply new configuration changes within a subshell
#(
#    unset BASHRCSOURCED
#    source ~/.bashrc
#)
